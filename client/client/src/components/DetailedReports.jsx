...
const DetailedReports = () => {
  ...
  // Calculations for current period (filtered data)
  const periodIncome = payments.reduce((acc, curr) => acc + curr.totalAmountPaid, 0);
  const periodExpense = expenses.reduce((acc, curr) => acc + curr.amount, 0);
  const periodNet = periodIncome - periodExpense;

  // Filter dues dynamically based on selected specific fund, selected fund type, and search query (Family ID / Name)
  const filteredDuesForExplorer = dues.filter(d => {
    ...
    // Live search query matching Family ID, Name, or Member ID
  ...
  return (
    <div className="reports-container animate-fade-in">
      ...
      {/* 1. Interactive Metrics Bar */}
      <div className="metrics-row">
        <div className="metric-card glass-panel text-teal">
          <div className="metric-icon"><TrendingUp size={24} /></div>
          <div className="metric-info">
            <span className="label">Total Collections (Selected Period)</span>
            <h3>₹{periodIncome.toLocaleString()}</h3>
          </div>
        </div>

        <div className="metric-card glass-panel text-pink">
          <div className="metric-icon"><TrendingDown size={24} /></div>
          <div className="metric-info">
            <span className="label">Total Expenses (Selected Period)</span>
            <h3>₹{periodExpense.toLocaleString()}</h3>
          </div>
        </div>

        <div className="metric-card glass-panel" style={{ color: periodNet >= 0 ? '#10b981' : '#ef4444' }}>
          <div className="metric-icon">{periodNet >= 0 ? <ArrowUpRight size={24} /> : <ArrowDownRight size={24} />}</div>
          <div className="metric-info">
            <span className="label">Net Period Profit/Loss</span>
            <h3>₹{periodNet.toLocaleString()}</h3>
          </div>
        </div>

        <div className="metric-card glass-panel text-indigo">
          <div className="metric-icon"><DollarSign size={24} /></div>
          <div className="metric-info">
            <span className="label">Outstanding Dues (All Time)</span>
            <h3>₹{summary.totalPendingDues.toLocaleString()}</h3>
          </div>
        </div>
      </div>
      ...
      <div className="tabs-header">
        ...
        >
          📈 Income / Collections ({payments.length})
        </button>
        <button 
        ...
        {loading ? (
          ...
          <>
            ...
            <table className="report-table">
              ...
              <tbody>
                {summary.duesBreakdown && summary.duesBreakdown.length > 0 && summary.duesBreakdown.some(d => d.pendingDues > 0) ? (
                  summary.duesBreakdown.filter(d => d.pendingDues > 0).map((db, idx) => (
            ...
            {activeTab === 'dues_explorer' && (
              <div className="dues-explorer-wrapper animate-fade-in">
                ...
                {/* A. Internal Tab Metrics Summary Row */}
                <div className="metrics-row" style={{ marginTop: '0', marginBottom: '24px' }}>
                  <div className="metric-card glass-panel text-teal" style={{ padding: '15px' }}>
                    <div className="metric-info">
                      <span className="label" style={{ fontSize: '0.75rem' }}>Expected Fund Amount</span>
                      <h4 style={{ margin: '5px 0 0 0', fontSize: '1.25rem', fontWeight: '800' }}>₹{auditExpectedTotal.toLocaleString()}</h4>
                    </div>
                  </div>
                  <div className="metric-card glass-panel text-emerald" style={{ padding: '15px' }}>
                    <div className="metric-info">
                      <span className="label" style={{ fontSize: '0.75rem' }}>Paid Total (Collections)</span>
                      <h4 style={{ margin: '5px 0 0 0', fontSize: '1.25rem', fontWeight: '800' }}>₹{auditPaidTotal.toLocaleString()}</h4>
                    </div>
                  </div>
                  <div className="metric-card glass-panel text-pink" style={{ padding: '15px' }}>
                    <div className="metric-info">
                      <span className="label" style={{ fontSize: '0.75rem' }}>Unpaid Total (Outstanding)</span>
                      <h4 style={{ margin: '5px 0 0 0', fontSize: '1.25rem', fontWeight: '800' }}>₹{auditUnpaidTotal.toLocaleString()}</h4>
                    </div>
                  </div>
                </div>
